...
public class QQShareActivity extends BaseActivity implements OnClickListener, OnCheckedChangeListener {
        ...
        switch (v.getId()) {
            case R.id.shareqq_commit: // 提交
                ...
                if (shareType != QQShare.SHARE_TO_QQ_TYPE_IMAGE) {
                	params.putString(QQShare.SHARE_TO_QQ_TITLE, title.getText().toString()); //标题
                    params.putString(QQShare.SHARE_TO_QQ_TARGET_URL, targetUrl.getText().toString());//分享的链接地址。
                    params.putString(QQShare.SHARE_TO_QQ_SUMMARY, summary.getText().toString());//分享的摘要文字，简述
                }
                            if (shareType == QQShare.SHARE_TO_QQ_TYPE_IMAGE) { //本地图片类型
    ...
    IUiListener qqShareListener = new IUiListener() {
        ...
        public void onCancel() {
            if (shareType != QQShare.SHARE_TO_QQ_TYPE_IMAGE) {
                Util.toastMessage(QQShareActivity.this, "onCancel:share1 ");
            }
        }
        @Override
